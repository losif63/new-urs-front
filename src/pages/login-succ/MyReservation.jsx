...
function ReservationInstance(props) {
  ...
  const deleteReservation = async () => {
    await axios.patch(`${process.env.REACT_APP_SERVER_URL}/reservation/cancelreservation`, {
      u_id: props.u_id,
      r_id: props.r_id
    });
    props.setReservations(props.reservations.filter((value, index, arr) => {
  ...
  return <div className="reservationInstance">
    <div className="textArea">
      ...
      <ReactModal isOpen={modalIsOpen} style={{
        ...
        <button onClick={() => setModalIsOpen(false)}><span aria-hidden="true">&times;</span></button>
        <div className="modalTitle">
          <text>{props.room_name}</text><br></br>
          <text>{props.building_name}</text><br></br>
        </div>
        <text>{props.room_name}</text><br></br>
        <text>{props.building_name}</text><br></br>
        <text>{props.dateInfo}</text><br></br>
        <text>{props.timeInfo}</text><br></br>
        <text>{props.username}</text><br></br>
        <text>{props.activity_name}</text><br></br>
...
function ReservationList(props) {
  ...
  return <div className="reservationList">
    {reservations.map(t => {
      return <ReservationInstance reservations={reservations} setReservations={setReservations} u_id={uId} r_id={t.r_id} room_name={t.room_name} building_name={t.building_name} dateInfo={t.start_time.substring(0, 10)} timeInfo={t.start_time.substring(11, 19) + ' ~ ' + t.finish_time.substring(11, 19)} username={t.username} activity_name={t.activity_name}></ReservationInstance>
    })}
  </div>;
}
